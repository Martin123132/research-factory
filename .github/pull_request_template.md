## Scope

- Workbench: `WB-NNN` or `FACTORY-WIDE`
- Contribution type: `HANGAR_CONSTRUCTION | EXPLORATION | NEGATIVE_RESULT | REPRODUCTION | DISPUTE`
- Accountable human:

## What changed and why

Describe the bounded change, the problem it addresses and the files or evidence
it affects.

## Measurement boundary

- Inputs and commitments:
- Independent verifier:
- Hard gates:
- Metrics and tolerance:
- Economic or physical guardrail:
- Known limitations:

## Verification

- [ ] Generated contracts and hashes are current.
- [ ] Relevant Python tests pass.
- [ ] Hangar typecheck, lint and tests pass when affected.
- [ ] No hidden inputs, answer sheets, credentials or personal data are included.
- [ ] Construction output is not presented as scientific evidence.
- [ ] I have read `IP_POLICY.md`, `CONTRIBUTOR_TERMS.md` and `LICENSING.md`.
- [ ] I have the right to submit every included component and declared material
      sources, licences, collaborators, institutional interests and AI use.
- [ ] The package is public and non-confidential; any necessary patent step
      occurred before upload.
- [ ] The contribution ledger names material roles and artifact references
      without assigning automatic solver, inventor or prize status.

## Rights and IP declaration

- Patent intent: `NONE | OPEN_COMMONS | FILED_BEFORE_PUBLICATION`
- Joint inventors considered: `YES | NOT_APPLICABLE`
- Progress-Friendly Patent Pledge intent:
  `EXPRESSED_INTENT | NOT_EXPRESSED | NOT_APPLICABLE`
- Freedom to operate: `NOT_ASSESSED | SUBMITTER_REVIEWED | COUNSEL_REVIEWED`
- Per-artifact licences or permissions:

## Reproduction status

- [ ] This pull request does not make a scientific promotion claim.
- [ ] Or: two reproductions operated by two distinct accountable humans are linked below, and
      neither validator is the author.

Validator A commitment:

Validator B commitment:

GitHub approvals are code review and do not satisfy the reproduction gate.
